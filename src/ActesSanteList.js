 ...
 const navigateTo = (route) => {
  ...
  const getActeSante = async () => {
    try {
      ...
      console.error('Erreur lors de la récupération des données de l\'acte de santé :', error);
    }
  };
    ...
    <div style={styles.container}>
      <div style={{...styles.header, ...(window.innerWidth < 768 ? styles.smallHeader : {})}}>
        ...
        <button style={{...styles.addButton, ...(window.innerWidth < 768 ? styles.smallAddButton : {})}}
                onClick={() => navigateTo('/admin-dashboard/add-actes-sante')}>
          Ajouter un acte de santé
        </button>
      </div>
      <div style={styles.content}>
        {actesantes.map((actesante) => (
          <div key={actesante.id} style={styles.card}>
            <div style={styles.cardHeader}>
              <h2 style={styles.cardHeaderText}>Acte de santé ID: {actesante.id}</h2>
            </div>
            <div style={styles.cardBody}>
              <p>Nom: {actesante.nom}</p>
              <p>Description: {actesante.description}</p>
              <p>Prix: {actesante.prix}</p>
              <p>Pays ID: {actesante.pays_id}</p>
              <div style={styles.actions}>
                <button style={{...styles.actionButton, marginRight: '10px'}}
                        onClick={() => navigateTo(`/admin-dashboard/edit-actes-sante/${actesante.id}`)}>
                  Modifier
                </button>
                <button style={{...styles.actionButton, ...styles.actionButtonDanger}}
                        onClick={() => handleDelete(actesante.id)}>
                  Supprimer
                </button>
              </div>
            </div>
